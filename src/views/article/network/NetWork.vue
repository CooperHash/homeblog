<template>
  <div class="netWok_page">
    <three>
      <template v-slot:navList>
        <li :style="validateList.validate1 ? enable : disable" @click="getAll">
          HTTP
        </li>
        <li
          :style="validateList.validate2 ? enable : disable"
          @click="getHttp1"
        >
          TCP
        </li>
        <li :style="validateList.validate3 ? enable : disable" @click="getTcp">
          UDP
        </li>
      </template>
...
export default {
  ...
  mounted: function () {
    ...
    console.log("初始化数组~");
    this.$http.get("/blog/api/getNetWork").then((res) => {
    ...
    this.$store.dispatch("setDefaultNetWork");
  ...
  methods: {
    ...
    toDiff(id) {
      ...
      if (id !== 0) {
        this.$store.dispatch("setNetWorkContent");
      } else {
        this.$store.dispatch("setDefaultNetWork");
      }
      switch (id) {
        ...
        case 1:
          this.array = [];
          this.$http.get("/blog/api/getHttp1").then((res) => {
            res.data.data.forEach((element) => {
              this.array.push(element);
            });
          });
        ...
        case 2:
          this.array = [];
          this.$http.get("/blog/api/getHttp2").then((res) => {
            res.data.data.forEach((element) => {
              this.array.push(element);
            });
          });
    ...
    getTcp() {
      this.array = [];
      this.$http.get("/blog/api/getTcp").then((res) => {
        res.data.data.forEach((item) => {
          this.array.push(item);
        });
      });
